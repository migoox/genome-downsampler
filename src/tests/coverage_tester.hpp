...
class CoverageTester : public SolverTester {
   public:
    using CoverageTestFunction = CoverageTestResult (*)(const std::unique_ptr<qmcp::Solver>&);

    void test(const std::unique_ptr<qmcp::Solver>& solver, fs::path& outputs_dir_path_) override;

   private:
    // tests
    static CoverageTestResult small_example_test(const std::unique_ptr<qmcp::Solver>& solver);
    static CoverageTestResult random_uniform_dist_test(const std::unique_ptr<qmcp::Solver>& solver);
    static CoverageTestResult random_low_coverage_on_both_sides_test(
        const std::unique_ptr<qmcp::Solver>& solver);
    static CoverageTestResult random_with_hole_test(const std::unique_ptr<qmcp::Solver>& solver);
    static CoverageTestResult random_zero_coverage_on_both_sides_test(
        const std::unique_ptr<qmcp::Solver>& solver);

    // helpers
    static void run_test_and_write_output(const std::unique_ptr<qmcp::Solver>& solver,
