...
void bam_api::AOSPairedReads::push_back(Read&& read) { reads.emplace_back(read); }

void bam_api::AOSPairedReads::push_back(const Read& read) { reads.push_back(read); }
...
bam_api::ReadIndex bam_api::AOSPairedReads::get_reads_count() const { return reads.size(); }

void bam_api::AOSPairedReads::reserve(size_t size) { reads.reserve(size); }

void bam_api::AOSPairedReads::clear() { reads.clear(); }
