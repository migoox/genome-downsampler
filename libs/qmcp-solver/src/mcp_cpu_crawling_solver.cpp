...
// #define DEBUG_PRINT
// ALGORYTM PEŁZAJĄCY
std::unique_ptr<qmcp::Solution> qmcp::McpCpuCrawlingSolver::solve(uint32_t max_coverage,
                                                                  bam_api::BamApi& bam_api)
{
    ...
    SizedPriorityQueue<HelperRead> best_M1(max_coverage), best_M2(max_coverage);
    SizedPriorityQueue<HelperRead>&best_M = best_M1, &best_M_prev = best_M2;
...
std::unique_ptr<qmcp::Solution> qmcp::McpCpuCrawlingSolver::obtain_sequence(
