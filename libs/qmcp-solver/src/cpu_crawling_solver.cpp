...
template <typename T>
void shift_right_from_index(std::vector<T>& vec, size_t index) {
...
struct SizedPriorityQueue {
    ...
    bool try_add(T val) {
        ...
        for (int x = m - 2; x >= 0; x--) {
            if (buffer[x] >= val) {
                shift_right_from_index(buffer, x + 1);
                buffer[x + 1] = val;
                return true;
            }
        }
        shift_right_from_index(buffer, 0);
        buffer[0] = val;
        return true;
    }
...
// test time 58s
void find_best_samples_try_find_shorter(
    uint32_t max_coverage, const bam_api::AOSPairedReads& input_sequence, uint32_t start_ind,
    uint32_t end_ind, std::vector<uint32_t>& start_index_count_pref_sum,
    ...
    for (uint32_t x = start_ind; x <= end_ind; x++) {
        ...
        for (int sorted_ind = subseq_last_sorted_ind;
            ...
            best_M.try_add(HelperRead(ind, input_sequence.reads[ind].end_ind));
        }
    }

    for (int x = 0; x < best_M_prev.size(); x++) best_M.try_add(best_M_prev[x]);
}

// test time 38s
void find_best_samples(uint32_t max_coverage, const bam_api::AOSPairedReads& input_sequence,
    ...
    do {
        ...
        for (uint32_t x = start_ind; x <= end_ind; x++) {
            ...
            found |= best_M.try_add(HelperRead(ind, input_sequence.reads[ind].end_ind));
        }
        ind_offset++;
    } while (found);
...
std::unique_ptr<qmcp::Solution> qmcp::CpuCrawlingSolver::solve(uint32_t max_coverage,
    ...
    const bam_api::AOSPairedReads& input_reads = bam_api.get_paired_reads_aos();
    ...
    for (int x = 0; x < reads_count; x++) {
        start_index_count_pref_sum[input_reads.reads[x].start_ind + 1]++;
    ...
    std::sort(read_ids_by_ascending_start_id.begin(), read_ids_by_ascending_start_id.end(),
              [&](int ind1, int ind2) {
                  if (input_reads.reads[ind1].start_ind == input_reads.reads[ind2].start_ind)
                      return input_reads.reads[ind1].end_ind < input_reads.reads[ind2].end_ind;
                  return input_reads.reads[ind1].start_ind < input_reads.reads[ind2].start_ind;
              });
    ...
    while (start_ind < input_reads.ref_genome_length) {
        ...
        //                                , best_M, best_M_prev);
        find_best_samples(max_coverage, input_reads, start_ind, end_ind, start_index_count_pref_sum,
        ...
        for (it = best_M[N - 1].endInd; N < index_samples_more_needed[it] &&
                                        it > input_reads.reads[best_M[N - 1].readInd].start_ind;
             it--)
            index_samples_more_needed[it] -= N;
        end_ind = it + 1;
        for (; it >= static_cast<int>(input_reads.reads[best_M[N - 1].readInd].start_ind); it--)
            index_samples_more_needed[it] = 0;
        best_M.erase_less(end_ind);
        best_M.erase_front(N);
